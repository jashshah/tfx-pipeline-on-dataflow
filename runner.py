...
from pipeline import create_pipeline                                                     # pylint: disable=import-error
...
if __name__ == '__main__':
  ...
  # Set the values for the compile time parameters
    ...
    beam_pipeline_args = [
        ...
        '--region=' + Config.GCP_REGION,
        '--no_use_public_ips',
        '--service_account_email=' + Config.CUSTOM_SERVICE_ACCOUNT,
        '--subnetwork=' + Config.SUBNETWORK,
        '--setup_file=' + Config.SETUP_FILE,
        '--job_name=' + Config.TFRECORD_JOB_NAME
    ]
    ...
    kubeflow_dag_runner.KubeflowDagRunner(config=runner_config).run(
        create_pipeline(
            ...
            metric_module_file = Config.METRIC_MODULE_FILE,
            path_to_tfrecords = Config.DATA_ROOT_URI,
            pipeline_name=os.environ["PIPELINE_NAME"],
            pipeline_root=PIPELINE_ROOT,
            train_steps=train_steps,
            eval_steps=eval_steps,
            ai_platform_distributed_training=strtobool(Config.ENABLE_AIP_TRAINING),
